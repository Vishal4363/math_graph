import type { Point, GridLine, ViewRange, PlotResult } from './types';
import { PLOT_SAMPLES, ZOOM_FACTOR } from './constants';
...
export function normalizeExpression(expr: string): string {
  ...
  while (prev !== e) {
    prev = e;
    e = e.replace(
      /([a-zA-Z_$][\w$.]*(?:\([^()]*\))?|\d+(?:\.\d+)?|\))\s*\^\s*(\((?:[^()]*|\([^()]*\))*\)|[a-zA-Z_$][\w$.]*(?:\([^()]*\))?|\d+(?:\.\d+)?|-\d+(?:\.\d+)?)/,
      'Math.pow($1,$2)'
    );
  }
...
export function buildFunction(expr: string): ((x: number) => number) | null {
  try {
    const normalized = normalizeExpression(expr);
    // eslint-disable-next-line no-new-func
    const fn = new Function('x', `"use strict"; ${HELPERS} return (${normalized});`) as (x: number) => number;
    fn(0);
    fn(1);
    return fn;
  } catch {
...
export function plotEquation(expr: string, view: ViewRange): PlotResult {
  ...
  const points: Point[] = [];
  const step = (view.xMax - view.xMin) / PLOT_SAMPLES;
...
export function mathToPixelX(mathX: number, view: ViewRange, width: number): number {
  return ((mathX - view.xMin) / (view.xMax - view.xMin)) * width;
}

export function mathToPixelY(mathY: number, view: ViewRange, height: number): number {
  return height - ((mathY - view.yMin) / (view.yMax - view.yMin)) * height;
}

export function pixelToMathX(pixelX: number, view: ViewRange, width: number): number {
  return view.xMin + (pixelX / width) * (view.xMax - view.xMin);
}

export function pixelToMathY(pixelY: number, view: ViewRange, height: number): number {
  return view.yMax - (pixelY / height) * (view.yMax - view.yMin);
}

// ----- Grid Lines -----

export function computeGridLines(min: number, max: number, pixelSize: number, axis: 'x' | 'y'): GridLine[] {
  const range = max - min;
...
export function zoomView(
  ...
  const factor = direction === 'in' ? 1 - ZOOM_FACTOR : 1 + ZOOM_FACTOR;
...
/** Pan by pixel delta */
export function panView(view: ViewRange, dx: number, dy: number, width: number, height: number): ViewRange {
